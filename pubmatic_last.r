...
y <- y %>% mutate( prebid = ifelse(MobileOrDesktopHTTP(as.character(V1)), createHTTP_Prebid_WEB(as.character(V1)) , createHTTP_Prebid_MOBILE(as.character(V1))) )
y <- merge(y,placementTypes,by="type")
y <- y %>% mutate( glue = glue('{V1}_{type}_{V2}_{id_type}_{size}'))
