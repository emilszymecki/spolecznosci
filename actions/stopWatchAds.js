...
let data  = $('.spolecznoscinet').toArray().map( x => 
  ({name: $(x).attr('id'),visible:$(x).isInViewport(),time:new Stopwatch,node : x})
)


const runner = () => {
  data.forEach( x => {
    x.visible = $(x.node).isInViewport()
    x.visible == true ? x.time.start() : x.time.stop()
    x.counter = x.time.toString() || 0
  })
  console.log(data)
}
...
$( window ).scroll(runner)


data.forEach( x => console.log(x.name, x.time.toString()))
