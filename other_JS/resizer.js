...
const imgLoad = url => ({ width, height, move, to, direct }) => {
	...
	img.onload = () => {
		...
		const directCtx =
			direct === "u"
				? [img, 0, -move, width, height]
				: direct === "d"
				? [img, 0, 0, width, height]
				: direct === "l"
				? [img, -move, 0, width, height]
				: direct === "r"
				? [img, 0, 0, width, height]
				: [img, 0, 0, width, height];
		const directCtx2 =
			direct === "u"
				? [img, 0, 0, width, height]
				: direct === "d"
				? [img, 0, -(height - move), width, height]
				: direct === "l"
				? [img, 0, 0, width, height]
				: direct === "r"
				? [img, -(width - move), 0, width, height]
				: [img, 0, 0, width, height];
		const directAllCanvas2 =
			direct === "u"
				? [canvas2, 0, 0, width, to]
				: direct === "d"
				? [canvas2, 0, height - move, width, to]
				: direct === "l"
				? [canvas2, 0, 0, to, height]
				: direct === "r"
				? [canvas2, width - move, 0, to, height]
				: [canvas2, 0, 0, width, to];
		const directAllCanvas =
			direct === "u"
				? [canvas, 0, to, width, height]
				: direct === "d"
				? [canvas, 0, 0, width, height - move]
				: direct === "l"
				? [canvas, to, 0, width, height]
				: direct === "r"
				? [canvas, 0, 0, width - move, height]
				: [canvas, 0, to, width, height];
		...
		canvas.width =
			direct === "l"
				...
				: width;
		canvas.height =
			direct === "u" ? height : direct === "d" ? height - move : height;
